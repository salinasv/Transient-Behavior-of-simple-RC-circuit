...
Area( cap1_left_limit:cap1_right_limit , cap_up_limit:cap_down_limit  ) = S_COPPER;
	%right side
cap2_right_limit = int32(center + lengthA(1)*0.05 + width_cell);
cap2_left_limit = int32(center + lengthA(1)*0.05);
Area( cap2_left_limit:cap2_right_limit , cap_up_limit:cap_down_limit  ) = S_COPPER;
% Charge capacitor
...
Area( wire_left:cap1_left_limit, wire_up:wire_down) = S_COPPER;
Area( cap2_right_limit:wire_right, wire_up:wire_down) = S_COPPER;
	% bottom wire
wire_b_up = int32(cap_down_limit + lengthA * 0.1);
wire_b_down = wire_b_up + WIRE_WIDTH * CELL_MM;
Area( wire_left:wire_right, wire_b_up: wire_b_down) = S_COPPER;
	% up-down wires
wire_ud_rigth = wire_left + WIRE_WIDTH * CELL_MM;
wire_ud_left = wire_right - WIRE_WIDTH * CELL_MM;
Area( wire_left:wire_ud_rigth, wire_up:wire_b_down) = S_COPPER;
Area( wire_ud_left:wire_right, wire_up:wire_b_down) = S_COPPER;
